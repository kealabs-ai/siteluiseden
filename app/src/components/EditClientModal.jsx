...
export function EditClientModal({ isOpen, onClose, clientData = {} }) {
  ...
  useEffect(() => {
    if (clientData && clientData.id) {
      setFormData({
        ...
        birthDate: clientData.data_aniversario || '',
        cep: clientData.cep || '',
        cidade: clientData.cidade || '',
        estado: clientData.estado || '',
        condominio: clientData.condominio || '',
        numero: clientData.numero || ''
      })
      if (clientData.data_aniversario) {
        const [day, month, year] = clientData.data_aniversario.split('/').map(Number)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await clientsApi.update(clientData.id, {
        nome: formData.name,
      ...
      addToast(`Cliente "${formData.name}" atualizado com sucesso!`, 'success')
      window.dispatchEvent(new CustomEvent('eden:data-changed', { detail: { resource: 'clients' } }))
      onClose()
    } catch (error) {
      addToast(getApiError(error, 'Não foi possível atualizar o cliente.'), 'error')
    }
  }
